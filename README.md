...
배포본에는 빌드 시점의 공유 그림체 게시글·프롬프트·원본 게시글 주소만 기본 데이터로 포함되며 이미지 파일은 포함되지 않습니다. 로컬 이미지가 없는 동안 원본 이미지 URL도 화면에 노출하지 않습니다. Chrome 로그인을 연결한 뒤 `용량·시간 확인`을 누르면 원본 게시글에서 만료되지 않은 이미지 URL을 다시 받아 누락 장수·예상 용량·예상 시간을 계산하며, 아직 이미지는 다운로드하지 않습니다. 확인 결과 아래의 `다운로드 시작`을 다시 눌러야 NovelAI PNG를 최대 16개씩 병렬로 받습니다. 중단 후 다시 실행하면 남은 이미지만 주소를 갱신해 이어받습니다.
